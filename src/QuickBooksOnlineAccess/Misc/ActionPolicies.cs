...
namespace QuickBooksOnlineAccess.Misc
{
	internal static class ActionPolicies
	{
		...
		{
			QuickBooksOnlineLogger.Log().Trace( ex, "Retrying Magento API submit call for the {0} time", i );
		...
		{
			QuickBooksOnlineLogger.Log().Trace( ex, "Retrying Magento API get call for the {0} time", i );
			SystemUtil.Sleep( TimeSpan.FromSeconds( 0.5 + i ) );
		} );
